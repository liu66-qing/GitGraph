...
@celery_app.task(bind=True, name="evograph.ingest_url")
def ingest_url_task(self, document_id: str, url: str):
    ...
    loop = asyncio.new_event_loop()
    try:
        from evograph.ingestion.url_loader import URLLoader
        loader = URLLoader()
...
@celery_app.task(bind=True, name="evograph.ingest_douyin")
